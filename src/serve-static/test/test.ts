...
describe('serveStatic()', function () {
  describe('basic operations', function () {
    ...
    it('should require root path', function () {
      assert.throws((serveStatic as any).bind(), /root path required/)
    })

    it('should require root path to be string', function () {
      assert.throws(serveStatic.bind(null, 42 as any), /root path.*string/)
    })
  ...
  describe('extensions', function () {
    ...
    it('should be configurable', function (done) {
      const server = createServer(fixtures, { extensions: 'txt' as any })
  ...
  describe('redirect', function () {
    let server: http.Server
    before(function () {
      server = createServer(fixtures, undefined, function (req, res) {
  ...
  describe('setHeaders', function () {
    it('should reject non-functions', function () {
      assert.throws(serveStatic.bind(null, fixtures, { setHeaders: 3 as any }), /setHeaders.*function/)
    })
  ...
  describe('when index at mount point', function () {
    let server: http.Server
    before(function () {
      server = createServer(fixtures + '/users', undefined, function (req) {
  ...
  describe('when mounted', function () {
    let server: http.Server
    before(function () {
      server = createServer(fixtures, undefined, function (req) {
        req.originalUrl = req.url
  ...
  describe('when mounted "root" as a file', function () {
    let server: http.Server
    before(function () {
      server = createServer(fixtures + '/todo.txt', undefined, function (req) {
        req.originalUrl = req.url
        req.url = '/' + req.url!.split('/').slice(2).join('/')
      })
    })
...
function createServer (dir?: string, opts?: any, fn?: (req: http.IncomingMessage, res?: http.ServerResponse) => void): http.Server {
  dir = dir || fixtures
  ...
  return http.createServer(function (req, res) {
    fn && fn(req, res)
    _serve(req, res, function (err?: any) {
