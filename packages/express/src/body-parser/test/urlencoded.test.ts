...
import * as bodyParser from '../src/index.js'
...
describe('bodyParser.urlencoded()', function () {
  ...
  it('should throw on invalid defaultCharset', function () {
    assert.throws(createServer.bind(null, { defaultCharset: 'utf-16' }),
      /TypeError: option defaultCharset must be either utf-8 or iso-8859-1/)
  })
  ...
  it('should handle duplicated middleware', function (done) {
    const urlencodedParser = bodyParser.urlencoded()
    const server = createServer(function (req: any, res: any, next: any) {
      urlencodedParser(req, res, function (err: any) {
    ...
    describe('when custom value set', function () {
      it('should reject non positive numbers', function () {
        ...
        assert.throws(createServer.bind(null, { extended: true, depth: NaN }),
          /TypeError: option depth must be a zero or a positive number/)
        assert.throws(createServer.bind(null, { extended: true, depth: 'beep' as any }),
          /TypeError: option depth must be a zero or a positive number/)
      })
  ...
  describe('with parameterLimit option', function () {
    describe('with extended: false', function () {
      ...
      it('should reject string', function () {
        assert.throws(createServer.bind(null, { extended: false, parameterLimit: 'beep' as any }),
          /TypeError: option parameterLimit must be a positive number/)
      })
    ...
    describe('with extended: true', function () {
      ...
      it('should reject string', function () {
        assert.throws(createServer.bind(null, { extended: true, parameterLimit: 'beep' as any }),
          /TypeError: option parameterLimit must be a positive number/)
      })
  ...
  describe('with verify option', function () {
    it('should assert value if function', function () {
      assert.throws(createServer.bind(null, { verify: 'lol' as any }),
  ...
  describe('async local storage', function () {
    before(function () {
      ...
      this.server = createServer(function (req: any, res: any, next: any) {
        const asyncLocalStorage = new AsyncLocalStorage()

        asyncLocalStorage.run(store, function () {
          urlencodedParser(req, res, function (err: any) {
            const local = asyncLocalStorage.getStore() as any
...
function createServer (opts?: any): Server {
