...
describe('normalizeOptions(options, defaultType)', () => {
  ...
  it('should remove additional options', () => {
    ...
    assert.strictEqual(result.defaultCharset, 'utf-8')
    assert.strictEqual(typeof result.shouldParse, 'function')
    assert.strictEqual((result as any).additional, undefined)
    assert.strictEqual((result as any).something, undefined)
  })
  ...
  describe('options', () => {
    describe('verify', () => {
      it('should throw an error if verify is not a function', () => {
        assert.throws(() => {
          normalizeOptions({ verify: 'not a function' as any }, 'application/json')
    ...
    describe('type', () => {
      it('should return the default type if type is not provided', () => {
        const result = normalizeOptions({}, 'application/json')
        assert.strictEqual(result.shouldParse({ headers: { 'content-type': 'application/json', 'content-length': '1024' } } as any), true)
        assert.strictEqual(result.shouldParse({ headers: { 'content-type': 'application/xml', 'content-length': '1024' } } as any), false)
      })

      it('should accept a string type', () => {
        const result = normalizeOptions({ type: 'application/xml' }, 'application/json')
        assert.strictEqual(result.shouldParse({ headers: { 'content-type': 'application/xml', 'content-length': '1024' } } as any), true)
        assert.strictEqual(result.shouldParse({ headers: { 'content-type': 'application/json', 'content-length': '1024' } } as any), false)
      })
      ...
      it('should accept an array of strings type', () => {
        ...
        assert.strictEqual(result.shouldParse({ headers: { 'content-type': 'application/xml', 'content-length': '1024' } } as any), true)
        assert.strictEqual(result.shouldParse({ headers: { 'content-type': 'application/ld+json', 'content-length': '1024' } } as any), true)
        assert.strictEqual(result.shouldParse({ headers: { 'content-type': 'application/json', 'content-length': '1024' } } as any), false)
      })

      it('should accept a type checking function', () => {
        const result = normalizeOptions({ type: () => true }, 'application/json')
        assert.strictEqual(result.shouldParse({ headers: { 'content-type': 'application/xml' } } as any), true)
        assert.strictEqual(result.shouldParse({ headers: { 'content-type': 'application/json' } } as any), true)
      })
    })
  ...
  describe('defaultType', () => {
    it('should throw an error if defaultType is not provided', () => {
      assert.throws(() => {
        normalizeOptions({}, undefined as any)
    ...
    it('should convert string defaultType to a request content-type checking function', () => {
      ...
      assert.strictEqual(typeof result.shouldParse, 'function')
      assert.strictEqual(result.shouldParse({ headers: { 'content-type': 'application/json', 'content-length': '1024' } } as any), true)
      assert.strictEqual(result.shouldParse({ headers: { 'content-type': 'application/xml', 'content-length': '100' } } as any), false)
    })
    ...
    it('should use function defaultType directly as the request content-type checker', () => {
      const typeFunction = (req: any) => req.headers['content-type'].endsWith('+json')
      const result = normalizeOptions({}, typeFunction)
      assert.strictEqual(result.shouldParse, typeFunction)
      assert.strictEqual(result.shouldParse({ headers: { 'content-type': 'application/ld+json' } } as any), true)
      assert.strictEqual(result.shouldParse({ headers: { 'content-type': 'application/xml' } } as any), false)
    })
  })
})
