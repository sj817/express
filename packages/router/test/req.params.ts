...
describe('req.params', function () {
  it('should default to empty object', function (done: any) {
    const router = Router()
  ...
  it('should not exist outside the router', function (done: any) {
    const router = Router()
  ...
  it('should overwrite value outside the router', function (done: any) {
    const router = Router()
    const server = createServer(function (req: any, res: any, next: any) {
      req.params = { foo: 'bar' }
      router(req, res, done)
  ...
  it('should restore previous value outside the router', function (done: any) {
    const router = Router()
    const server = createServer(function (req: any, res: any, next: any) {
      req.params = { foo: 'bar' }
  ...
  describe('when "mergeParams: true"', function () {
    it('should merge outside object with params', function (done: any) {
    ...
    it('should ignore non-object outside object', function (done: any) {
      const router = Router({ mergeParams: true })
      const server = createServer(function (req: any, res: any, next: any) {
        req.params = 42
    ...
    it('should overwrite outside keys that are the same', function (done: any) {
      const router = Router({ mergeParams: true })
      const server = createServer(function (req: any, res: any, next: any) {
        req.params = { foo: 'bar' }
    ...
    describe('with numeric properties in req.params', function () {
      it('should merge numeric properties by offsetting', function (done: any) {
        const router = Router({ mergeParams: true })
        const server = createServer(function (req: any, res: any, next: any) {
      ...
      it('should merge with same numeric properties', function (done: any) {
        const router = Router({ mergeParams: true })
        const server = createServer(function (req: any, res: any, next: any) {
          req.params = { 0: 'foo' }
...
function sawParams (req: any, res: any) {
  res.statusCode = 200
