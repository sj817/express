...
describe('Router', function () {
  ...
  it('should support .use of other routers', function (done: any) {
    const router = new Router()
    const another = new Router()

    another.get('/bar', function (req: any, res: any) {
      res.end()
    })
  ...
  it('should support dynamic routes', function (done: any) {
    const router = new Router()
    const another = new Router()
  ...
  it('should handle blank URL', function (done: any) {
    const router = new Router()
  ...
  it('should handle missing URL', function (done: any) {
    const router = new Router()
  ...
  it('handle missing method', function (done: any) {
    let all = false
    const router = new Router()
    const route = router.route('/foo')
    let use = false

    route.post(function (req: any, res: any, next: any) { next(new Error('should not run')) })
    route.all(function (req: any, res: any, next: any) {
      all = true
      next()
    })
    route.get(function (req: any, res: any, next: any) { next(new Error('should not run')) })

    router.get('/foo', function (req: any, res: any, next: any) { next(new Error('should not run')) })
    router.use(function (req: any, res: any, next: any) {
      use = true
      next()
    })

    router.handle({ url: '/foo' }, {}, function (err: any) {
  ...
  it('should not stack overflow with many registered routes', function (done: any) {
    ...
    const handler = function (req: any, res: any) { res.end(new Error('wrong handler')) }
    const router = new Router()
  ...
  it('should not stack overflow with a large sync route stack', function (done: any) {
    this.timeout(5000) // long-running test

    const router = new Router()

    router.get('/foo', function (req: any, res: any, next: any) {
      req.counter = 0
      next()
    })
    ...
    router.handle({ url: '/foo', method: 'GET' }, { end: done }, function (err: any) {
      assert(!err, err)
    })
  })

  it('should not stack overflow with a large sync middleware stack', function (done: any) {
    this.timeout(5000) // long-running test
    ...
    router.handle({ url: '/', method: 'GET' }, { end: done }, function (err: any) {
      assert(!err, err)
    })
  })

  describe('.handle', function () {
    it('should dispatch', function (done: any) {
      const router = new Router()

      router.route('/foo').get(function (req: any, res: any) {
        res.send('foo')
      })
  ...
  describe('error', function () {
    it('should skip non error middleware', function (done: any) {
      const router = new Router()
      ...
      router.get('/bar', function (req: any, res: any, next: any) {
        next(new Error('bar'))
      })
    ...
    it('should handle throwing inside routes with params', function (done: any) {
      const router = new Router()
    ...
    it('should handle throwing in handler after async param', function (done: any) {
      const router = new Router()
      ...
      router.use('/:user', function (req: any, res: any, next: any) {
  ...
  describe('FQDN', function () {
    ...
    it('should ignore FQDN in search', function (done: any) {
      ...
      router.use('/proxy', function (req: any, res: any, next: any) {
        assert.equal(req.hit++, 0)
    ...
    it('should adjust FQDN req.url', function (done: any) {
      const request = { hit: 0, url: 'http://example.com/blog/post/1', method: 'GET' }
      const router = new Router()

      router.use('/blog', function (req: any, res: any, next: any) {
        assert.equal(req.hit++, 0)
    ...
    it('should adjust FQDN req.url with multiple handlers', function (done: any) {
      const request = { hit: 0, url: 'http://example.com/blog/post/1', method: 'GET' }
      const router = new Router()
    ...
    it('should adjust FQDN req.url with multiple routed handlers', function (done: any) {
      const request = { hit: 0, url: 'http://example.com/blog/post/1', method: 'GET' }
      const router = new Router()
  ...
  describe('.all', function () {
    it('should support using .all to capture all http verbs', function (done: any) {
      const router = new Router()
      ...
      methods.forEach(function testMethod (method) {
        router.handle({ url, method }, {}, function () { })
      })
  ...
  describe('.use', function () {
    ...
    it('should be called for any URL', function (done: any) {
    ...
    it('should accept array of middleware', function (done: any) {
      let count = 0
      const router = new Router()
      ...
      router.use([fn1, fn2], function (req: any, res: any) {
  ...
  describe('.param', function () {
    ...
    it('should call param function when routing VERBS', function (done: any) {
      const router = new Router()
      ...
      router.get('/foo/:id/bar', function (req: any, res: any, next: any) {
    ...
    it('should call param function when routing middleware', function (done: any) {
      const router = new Router()
      ...
      router.use('/foo/:id/bar', function (req: any, res: any, next: any) {
        assert.equal(req.params.id, '123')
    ...
    it('should only call once per request', function (done: any) {
      ...
      sub.get('/bar', function (req: any, res: any, next: any) {
        next()
      })
      ...
      router.handle(req, {}, function (err: any) {
        if (err) return done(err)
        assert.equal(count, 1)
  ...
  describe('parallel requests', function () {
    it('should not mix requests', function (done: any) {
